...
def prepare_asset(request, unique_name=False):
    ...
    asset = {
        ...
        'announcement_id': get('announcement_id'),
    ...
    if not asset['announcement_id']:
        asset['announcement_id'] = uuid.uuid4().hex
        if uri.startswith('/'):
            rename(uri, path.join(settings['assetdir'], asset['announcement_id']))
            uri = path.join(settings['assetdir'], asset['announcement_id'])

    if 'youtube_asset' in asset['mimetype']:
        uri, asset['name'], asset['duration'] = download_video_from_youtube(uri, asset['announcement_id'])
...
def prepare_asset_v1_2(request_environ, announcement_id=None, unique_name=False):
...
def prepare_usb_asset(filepath, **kwargs):
    ...
    if kwargs['copy']:
        shutil.copy(filepath, path.join(settings['assetdir'], announcement_id))
        filepath = path.join(settings['assetdir'], announcement_id)
...
def prepare_default_asset(**kwargs):
    ...
    announcement_id = 'default_{}'.format(uuid.uuid4().hex)
...
def remove_default_assets():
    ...
    with db.conn(settings['database']) as conn:
        for asset in assets_helper.read(conn):
            if asset['announcement_id'].startswith('default_'):
                assets_helper.delete(conn, asset['announcement_id'])
...
def update_asset(asset, data):
    for key, value in data.items():
        ...
        if key in ['announcement_id', 'is_processing', 'mimetype', 'uri'] or key not in asset:
            continue

        if key in ['start_date', 'end_date']:
            value = date_parser.parse(value).replace(tzinfo=None)

        if key in ['play_order', 'is_enabled', 'is_active']:
            value = int(value)
...
class Asset(Resource):
    ...
    def get(self, announcement_id):
        with db.conn(settings['database']) as conn:
            return assets_helper.read(conn, announcement_id)
        ...
        'parameters': [
            ...
            },
            {
                ...
                'description':
                    ...
                    Content-Type: application/x-www-form-urlencoded
                    model: "{
                        "announcement_id": "793406aa1fd34b85aa82614004c0e63a",
                        "name": "Website",
                        "mimetype": "webpage",
                        "uri": "http://example.com",
                        "is_active": 0,
    ...
    def put(self, announcement_id):
        with db.conn(settings['database']) as conn:
            return assets_helper.update(conn, announcement_id, prepare_asset(request))
    ...
    def delete(self, announcement_id):
        with db.conn(settings['database']) as conn:
            asset = assets_helper.read(conn, announcement_id)
            try:
                if asset['uri'].startswith(settings['assetdir']):
                    remove(asset['uri'])
            except OSError:
                pass
            assets_helper.delete(conn, announcement_id)
...
class AssetsV1_2(Resource):
    ...
    def post(self):
        ...
        if not asset['name'] and url_fails(asset['uri']):
            raise Exception("Could not retrieve file. Check the asset URL.")
        with db.conn(settings['database']) as conn:
            assets = assets_helper.read(conn)
            ids_of_active_assets = [x['announcement_id'] for x in assets if x['is_active']]

            asset = assets_helper.create(conn, asset)

            if asset['is_active']:
                ids_of_active_assets.insert(asset['play_order'], asset['announcement_id'])
            assets_helper.save_ordering(conn, ids_of_active_assets)
            return assets_helper.read(conn, asset['announcement_id']), 201
...
class AssetV1_2(Resource):
    ...
    def patch(self, announcement_id):
        ...
        with db.conn(settings['database']) as conn:
            ...
            asset = assets_helper.update(conn, announcement_id, asset)

            try:
                ids_of_active_assets.remove(asset['announcement_id'])
            except ValueError:
                pass
            if asset['is_active']:
                ids_of_active_assets.insert(asset['play_order'], asset['announcement_id'])
    ...
    def put(self, announcement_id):
        asset = prepare_asset_v1_2(request, announcement_id)
        with db.conn(settings['database']) as conn:
            assets = assets_helper.read(conn)
            ids_of_active_assets = [x['announcement_id'] for x in assets if x['is_active']]
        ...
        'parameters': [
            {
                ...
                'description':
                    ...
                    asset&announcement_id - show asset with `announcement_id` id
                    '''
            }
        ],
        'responses': {
...
class ViewerCurrentAsset(Resource):
    ...
    def get(self):
        ...
        publisher = ZmqPublisher.get_instance()
        publisher.send_to_viewer('current_announcement_id')

        collector_result = collector.recv_json(2000)
        current_announcement_id = collector_result.get('current_announcement_id')

        if not current_announcement_id:
            return []

        with db.conn(settings['database']) as conn:
            return assets_helper.read(conn, current_announcement_id)


api.add_resource(Assets, '/api/v1/assets')
api.add_resource(Asset, '/api/v1/assets/<announcement_id>')
api.add_resource(AssetsV1_1, '/api/v1.1/assets')
api.add_resource(AssetV1_1, '/api/v1.1/assets/<announcement_id>')
api.add_resource(AssetsV1_2, '/api/v1.2/assets')
api.add_resource(AssetV1_2, '/api/v1.2/assets/<announcement_id>')
api.add_resource(AssetContent, '/api/v1/assets/<announcement_id>/content')
