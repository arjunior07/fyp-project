...
FIELDS = ["announce_id", "name", "uri", "start_date",
          "end_date", "duration", "mimetype", "is_enabled", "play_order"]

create_assets_table = 'CREATE TABLE assets(announce_id text primary key, name text, uri text, start_date timestamp, end_date timestamp, duration text, mimetype text, is_enabled integer default 0,  play_order integer default 0)'
...
def is_active(asset, at_time=None):
    ...
    >>> asset = {'announce_id': u'4c8dbce552edb5812d3a866cfe5f159d', 'mimetype': u'web', 'name': u'WireLoad', 'end_date': datetime.datetime(2013, 1, 19, 23, 59), 'uri': u'http://www.wireload.net', 'duration': u'5', 'is_enabled': True, 'nocache': 0, 'play_order': 1, 'start_date': datetime.datetime(2013, 1, 16, 0, 0), 'skip_asset_check': 0};
...
def read(conn, announce_id=None, keys=FIELDS):
    """
    ...
    with db.cursor(conn) as c:
        if announce_id is None:
            ...
            c.execute(queries.read(keys), [announce_id])
        assets = [mk(asset) for asset in c.fetchall()]
    [asset.update({'is_active': is_active(asset)}) for asset in assets]
    if announce_id and len(assets):
        return assets[0]
    return assets


def update(conn, announce_id, asset):
    """
    Update an asset in the database.
    Returns the asset.
    Asset's announce_id and is_active field is updated before returning.
    """
    if asset.get('announce_id'):
        del asset['announce_id']
    if 'is_active' in asset:
        asset.pop('is_active')
    with db.commit(conn) as c:
        c.execute(queries.update(asset.keys()), asset.values() + [announce_id])
    asset.update({'announce_id': announce_id})
    if 'start_date' in asset:
        asset.update({'is_active': is_active(asset)})
    return asset


def delete(conn, announce_id):
    """Remove an asset from the database."""
    with db.commit(conn) as c:
        c.execute(queries.remove, [announce_id])
    ...
    if ids:
        with db.commit(db_conn) as c:
            c.execute(queries.multiple_update_with_case(['play_order', ], len(ids)),
                      sum([[announce_id, play_order] for play_order, announce_id in enumerate(ids)], []) + ids)
