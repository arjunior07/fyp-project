...
def navigate_to_asset(announcement_id):
    scheduler.extra_asset = announcement_id
    system('pkill -SIGUSR1 -f viewer.py')
...
def send_current_announcement_id_to_server():
    consumer = ZmqConsumer()
    consumer.send({'current_announcement_id': scheduler.current_announcement_id})
...
commands = {
    ...
    'current_announcement_id': lambda _: send_current_announcement_id_to_server()
}
    ...
    def get_next_asset(self):
        ...
        if self.extra_asset is not None:
            ...
            if asset and asset['is_processing'] == 0:
                self.current_announcement_id = self.extra_asset
                self.extra_asset = None
                return asset
        ...
        self.current_announcement_id = current_asset.get('announcement_id')
...
def get_specific_asset(announcement_id):
    logging.info('Getting specific asset')
    return assets_helper.read(db_conn, announcement_id)
