...
asset_w = {
    'mimetype': u'web',
    'announce_id': u'4c8dbce552edb5812d3a866cfe5f159e',
    'name': u'いろはにほへど',
...
asset_x = {
    'mimetype': u'web',
    'announce_id': u'4c8dbce552edb5812d3a866cfe5f159d',
    'name': u'WireLoad',
    'uri': u'http://www.wireload.net',
    'start_date': date_a,
    'end_date': date_b,
...
asset_y = {
    'mimetype': u'image',
    'announce_id': u'7e978f8c1204a6f70770a1eb54a76e9b',
    'name': u'Google',
...
asset_z = {
    'mimetype': u'image',
    'announce_id': u'9722cd9c45e44dc9b23521be8132b38f',
    'name': u'url test',
    'start_date': date_c,
    'end_date': date_d,
    'duration': u'1',
...
class DBHelperTest(unittest.TestCase):
    ...
    def test_create_update_read_asset(self):
        ...
        assets_helper.update(self.conn, asset_x['announce_id'], asset_x_)

        assets_helper.create(self.conn, asset_y)
        asset_y_ = asset_y.copy()
        asset_y_.update(**asset_y_diff)
        assets_helper.update(self.conn, asset_y['announce_id'], asset_y_)
    ...
    def test_create_delete_asset(self):
        assets_helper.create(self.conn, asset_x)
        assets_helper.delete(self.conn, asset_x['announce_id'])

        assets_helper.create(self.conn, asset_y)
        assets_helper.delete(self.conn, asset_y['announce_id'])
    ...
    def test_create_update_read_asset_utf8(self):
        ...
        assets_helper.update(self.conn, asset_w['announce_id'], asset_w_)
    ...
    def test_get_playlist(self):
        ...
        self.assertEqual(asset_x['announce_id'], should_be_x['announce_id'])

        self.set_now(date_g)
        should_be_y_x = assets_helper.get_playlist(self.conn)
        self.assertEqual([should_be_y_x[0]['announce_id'],
                          should_be_y_x[1]['announce_id']],
                         [asset_y['announce_id'],
                          asset_x['announce_id']])

        self.set_now(date_h)
        [should_be_y] = assets_helper.get_playlist(self.conn)
        self.assertEqual(asset_y['announce_id'], should_be_y['announce_id'])
        # ✂--------
    ...
    def test_set_order(self):
        ...
        assets_helper.save_ordering(self.conn, [asset['announce_id'] for asset in for_order])

        fetched = assets_helper.read(self.conn)

        self.assertEquals(
            [(0, asset_y['announce_id']),
             (1, asset_x['announce_id']),
             (2, asset_z['announce_id']),
             (2, asset_w['announce_id'])],
            [(asset['play_order'], asset['announce_id']) for asset in fetched])
